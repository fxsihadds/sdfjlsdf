...
def extract_user_passwords(text):
    email_passwords = re.findall(regex, text)
    return email_passwords


@Client.on_message(filters.command('uscr'))
async def scraper_command(client, message):
    ...
    if message.reply_to_message:
        if message.reply_to_message.document:
            ...
            if document.file_name.endswith('.txt'):
                ...
                await status.edit_text(text='<b>Please provide a text file (.txt) to scrape User:password combinations😞.</b>')
                return
    ...
    if text_to_scrape is None:
        await status.edit_text(text='<b>No text found to scrape User:password combinations😞.</b>')
        return
    ...
    if len(email_passwords) > 0:
        ...
        await client.send_document(
            ...
            document=filename,
            caption=f'<b>Extracted {len(email_passwords)} User:password</b>'
        )
        ...
        await status.edit_text(text='<b>No User:password combinations found in the provided text ❌.</b>')
